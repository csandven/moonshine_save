...
fn spawn_button(parent: &mut ChildBuilder, asset_server: &AssetServer, value: impl Into<String>, bundle: impl Bundle) {
  parent
    .spawn((
      bundle,
      ButtonBundle {
...
fn update_text(
  ...
  let melee_count = soldiers
    .iter()
    .filter_map(|SoldierWeapon(entity)| entity.and_then(|weapon_entity| weapon_query.get(weapon_entity).ok()))
...
fn update_buttons(mut interaction_query: Query<(&Interaction, &mut BackgroundColor), Changed<Interaction>>) {
...
fn handle_add_melee(query: Query<&Interaction, (With<AddMeleeButton>, Changed<Interaction>)>, mut commands: Commands) {
  if let Ok(Interaction::Clicked) = query.get_single() {
...
fn handle_save(query: Query<&Interaction, (With<SaveButton>, Changed<Interaction>)>, mut commands: Commands) {
  if let Ok(Interaction::Clicked) = query.get_single() {
...
fn handle_load(query: Query<&Interaction, (With<LoadButton>, Changed<Interaction>)>, mut commands: Commands) {
  if let Ok(Interaction::Clicked) = query.get_single() {
