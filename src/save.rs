...
//!     .register_type::<Data>()
//!     .add_systems(save_into_file("example.ron"));
//!
...
impl Plugin for SavePlugin {
  fn build(&self, app: &mut App) {
    app
      .configure_sets(
        PreUpdate,
        (SaveSet::Save, SaveSet::PostSave.run_if(has_resource::<Saved>), SaveSet::Flush.run_if(has_resource::<Saved>))
          .chain(),
      )
      .add_systems(Update, (remove_resource::<Saved>, apply_deferred).in_set(SaveSet::Flush));
  }
}
...
pub fn into_file(path: PathBuf) -> impl Fn(In<Saved>, Res<AppTypeRegistry>) -> Result<Saved, Error> {
  ...
  (
    {
      ...
      #[cfg(feature = "hierarchy")]
      let s = s.pipe(forget_component::<bevy_hierarchy::Children>);
      s.pipe(file_from_request::<R>).pipe(into_file_dyn).pipe(finish)
    },
    remove_resource::<R>,
  )
    .chain()
...
where
  R: Event + SaveIntoFileRequest + Send + Sync + 'static,
{
  ...
  ({
    ...
    #[cfg(feature = "hierarchy")]
    let s = s.pipe(forget_component::<bevy_hierarchy::Children>);
    s.pipe(file_from_event::<R>).pipe(into_file_dyn).pipe(finish)
  },)
